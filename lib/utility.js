...
module.exports = {
  ...
  buildCommand: function(activeEditor, file, real_file) {
    ...
    if (atom.config.get("linter-gcc-portuguese.gccDebug")){
      console.log("linter-gcc-portuguese config: " + JSON.stringify(settings));
    }
    ...
    if (settings.compileCommandsFile.trim() != "") {
        ...
        try {
          compile_commands.forEach(function(item) {
            if (item.file == real_file) {
              ...
              if (atom.config.get("linter-gcc-portuguese.gccLintOnTheFly") == true) {
      ...
      if (!command) {
        atom.notifications.addError(
          "linter-gcc-portuguese: Executable not found", {
            detail: "\"" + settings.execPath + "\" not found"
          }
        );
        console.log("linter-gcc-portuguese: \"" + settings.execPath + "\" not found");
        return;
      }
    ...
    if (atom.config.get("linter-gcc-portuguese.gccDebug")){
      full_command = "linter-gcc-portuguese: " + command;
  ...
  parse: function(data,editor) {
    ...
    if (settings.gcc7orGreater) {
      var regex = `(?<file>.+):(?<line>\\d+):(?<col>\\d+):\\s*\\w*\\s*(?<type>(${atom.config.get("linter-gcc-portuguese.gccErrorString")}|${atom.config.get("linter-gcc-portuguese.gccWarningString")}|${atom.config.get("linter-gcc-portuguese.gccNoteString")}))\\s*:\\s*(?<message>.*)(\\r?\\n(?<code>.*)\\r?\\n(?<placemarks>\\s+\\^~*))*`
    }
    else
    {
      var regex = `(?<file>.+):(?<line>\\d+):(?<col>\\d+):\\s*\\w*\\s*(?<type>(${atom.config.get("linter-gcc-portuguese.gccErrorString")}|${atom.config.get("linter-gcc-portuguese.gccWarningString")}|${atom.config.get("linter-gcc-portuguese.gccNoteString")}))\\s*:\\s*(?<message>.*)`
    }
