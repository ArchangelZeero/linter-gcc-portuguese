...
module.exports = {
  ...
  lint: function(editor, linted_file, real_file){
    ...
    otherFiles = new Set([])
    if (atom.config.get("linter-gcc-portuguese.gccRelintMessageSources") == true){
    ...
    return require("sb-exec").exec(command.binary, command.args, {stream: "stderr"}).then(output => {
      ...
      msgs.forEach(function(entry){
        ...
        if (entry.severity ===`${atom.config.get("linter-gcc-portuguese.gccWarningString")}`)
            entry.severity = "warning";
        else if (entry.severity ===`${atom.config.get("linter-gcc-portuguese.gccErrorString")}`)
            entry.severity = "error";
        else if (entry.severity ===`${atom.config.get("linter-gcc-portuguese.gccNoteString")}`)
            entry.severity = "info";
      })
  ...
  consumeLinter: function(registerIndie) {
    ...
    lintOnTheFly = function() {
      ...
      if (!editor) return;
      if (atom.config.get("linter-gcc-portuguese.gccLintOnTheFly") == false) return;
      if (lint_waiting) return;
      lint_waiting = true
      interval = atom.config.get("linter-gcc-portuguese.gccLintOnTheFlyInterval")
    ...
    lintOnSave = function(){
      ...
      editor = utility.getValidEditor(atom.workspace.getActiveTextEditor());
      if (!editor) return;
      if (atom.config.get("linter-gcc-portuguese.gccLintOnTheFly") == true) return;
      real_file = editor.getPath();
      //cleanup messages in this file created by linting other files
      if (atom.config.get("linter-gcc-portuguese.gccRelintMessageSources") == false){
        for (var file in module.exports.messages){
          if (!module.exports.messages.hasOwnProperty(file)) continue;
          var fileMessages = module.exports.messages[file];
          for (var msg in fileMessages){
